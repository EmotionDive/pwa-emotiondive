...
const EmpathySympathyAntipathy2 = () => {
	...
	const handleGrade = () => {
		setCorrectAnswer(
			data.sets[currentSet].scenarios[currentScenario].answer_index
		)
	}
				...
				<main className='EmpathySympathyAntipathy2Activity'>
					<div className='scenario'>
						...
						<OptionButtonGroup
							...
							value={currentAnswer}
						>
							{data.sets[currentSet].scenarios[currentScenario].options.map(
								(text, index) => (
									<OptionButton
										value={opts[index]}
										key={index}
						...
						<div className='scenario__bottom'>
								...
								<div className='scenario__bottom__explication'>
									...
									{opts.indexOf(currentAnswer) === correctAnswer ? (
										<p className='justifyText'>
											Muy bien, lograste identificar el tipo de comportamiento
											en dicho escenario, parece que tienes claros los conceptos
											de <strong>empatía, simpatía y antipatía</strong> para
											este caso.
										</p>
									) : (
										<p className='justifyText'>
											Parece que fallastes, te recomendamos darle un repaso
											adicional a las definiciones de{' '}
											<strong>empatía, simpatía y antipatía.</strong>
										</p>
									)}
								</div>
							)}
							...
							{correctAnswer === null ? (
							  ...
							  data.sets[currentSet].scenarios.length - 1 ? (
